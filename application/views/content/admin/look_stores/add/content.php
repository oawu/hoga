  ...
  <table class='table-form'>
    <tbody>
      ...
      <?php if ($tags = LookStoreTag::all ()) {
              foreach ($tags as $tag) { ?>
                <option value='<?php echo $tag->id;?>'<?php echo (isset ($posts['look_store_tag_id']) ? $posts['look_store_tag_id'] : 0) == $tag->id ? ' selected': '';?>><?php echo $tag->name;?>(<?php echo $tag->en_name;?>)</option>
        <?php }
            } else { ?>
      ...
      <?php if ($isIsEnabledNames = LookStore::$isIsEnabledNames) {
              foreach ($isIsEnabledNames as $key => $name) { ?>
                <option value='<?php echo $key;?>'<?php echo (isset ($posts['is_enabled']) ? $posts['is_enabled'] : LookStore::NO_ENABLED) == $key ? ' selected': '';?>><?php echo $name;?></option>
        <?php }
            }?>
          </select>
        </td>
