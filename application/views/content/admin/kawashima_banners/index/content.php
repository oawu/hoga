...
<?php if ($banners) {
        foreach ($banners as $banner) { ?>
          <tr>
            <td data-title='Banner'><?php echo img($banner->name->url ('100x100c'));?></td>
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $banner->id;?>'<?php echo $banner->is_enabled == KawashimaBanner::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo KawashimaBanner::$isIsEnabledNames[$banner->is_enabled];?></div>
              </label>
            </td>
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $banner->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $banner->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
