...
<?php if ($brands) {
        foreach ($brands as $brand) { ?>
          <tr>
            <td data-title='封面' width='50'><?php echo img ($brand->cover->url ('100x100c'), false, 'class="i_30"');?></td>
            <td data-title='名稱' width='150'><?php echo $brand->name;?></td>
            <td data-title='描述' width='' class='left'><?php echo $brand->mini_content ();?></td>
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $brand->id;?>'<?php echo $brand->is_enabled == Brand::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo Brand::$isIsEnabledNames[$brand->is_enabled];?></div>
              </label>
            </td>
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $brand->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $brand->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
