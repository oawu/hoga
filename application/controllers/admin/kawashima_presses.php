...
class Kawashima_presses extends Admin_controller {
  private $uri_1 = null;
  private $press = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'kawashima_presses';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->press = KawashimaPress::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = KawashimaPress::count (array ('conditions' => $conditions));
    ...
    $presses = KawashimaPress::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => 'id DESC',
  ...
  public function create () {
    ...
    $press = null;
    $create = KawashimaPress::transaction (function () use (&$press, $posts, $big, $small) {
      if (!verifyCreateOrm ($press = KawashimaPress::create (array_intersect_key ($posts, KawashimaPress::table ()->columns))))
        return false;
  ...
  public function update () {
    ...
    $press = $this->press;
    $update = KawashimaPress::transaction (function () use ($press, $posts, $big, $small) {
      if (!$press->save ())
        return false;
  ...
  public function destroy () {
    $press = $this->press;
    $delete = KawashimaPress::transaction (function () use ($press) {
  ...
  public function is_enabled ($id = 0) {
    if (!($id && ($press = KawashimaPress::find_by_id ($id, array ('select' => 'id, is_enabled, updated_at')))))
      return $this->output_json (array ('status' => false, 'message' => '當案不存在，或者您的權限不夠喔！'));

    $posts = OAInput::post ();

    if ($msg = $this->_validation_is_enabled_posts ($posts))
      return $this->output_json (array ('status' => false, 'message' => $msg, 'content' => KawashimaPress::$isIsEnabledNames[$press->is_enabled]));

    if ($columns = array_intersect_key ($posts, $press->table ()->columns))
      foreach ($columns as $column => $value)
        $press->$column = $value;

    $update = KawashimaPress::transaction (function () use ($press) { return $press->save (); });

    if (!$update)
      return $this->output_json (array ('status' => false, 'message' => '更新失敗！', 'content' => KawashimaPress::$isIsEnabledNames[$press->is_enabled]));

    return $this->output_json (array ('status' => true, 'message' => '更新成功！', 'content' => KawashimaPress::$isIsEnabledNames[$press->is_enabled]));
  }
  ...
  private function _validation_is_enabled_posts (&$posts) {
    if (!(isset ($posts['is_enabled']) && is_numeric ($posts['is_enabled']) && in_array ($posts['is_enabled'], array_keys (KawashimaPress::$isIsEnabledNames))))
      return '參數錯誤！';
    return '';
  }
}
