...
class Look_banners extends Admin_controller {
  private $uri_1 = null;
  private $banner = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'look_banners';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->banner = LookBanner::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = LookBanner::count (array ('conditions' => $conditions));
    ...
    $banners = LookBanner::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => 'id DESC',
  ...
  public function create () {
    ...
    $banner = null;
    $create = LookBanner::transaction (function () use (&$banner, $posts, $name) {
      if (!verifyCreateOrm ($banner = LookBanner::create (array_intersect_key ($posts, LookBanner::table ()->columns))))
        return false;
  ...
  public function update () {
    ...
    $banner = $this->banner;
    $update = LookBanner::transaction (function () use ($banner, $posts, $name) {
      if (!$banner->save ())
        return false;
  ...
  public function destroy () {
    $banner = $this->banner;
    $delete = LookBanner::transaction (function () use ($banner) {
  ...
  public function is_enabled ($id = 0) {
    if (!($id && ($banner = LookBanner::find_by_id ($id, array ('select' => 'id, is_enabled, updated_at')))))
      return $this->output_json (array ('status' => false, 'message' => '當案不存在，或者您的權限不夠喔！'));

    $posts = OAInput::post ();

    if ($msg = $this->_validation_is_enabled_posts ($posts))
      return $this->output_json (array ('status' => false, 'message' => $msg, 'content' => LookBanner::$isIsEnabledNames[$banner->is_enabled]));

    if ($columns = array_intersect_key ($posts, $banner->table ()->columns))
      foreach ($columns as $column => $value)
        $banner->$column = $value;

    $update = LookBanner::transaction (function () use ($banner) { return $banner->save (); });

    if (!$update)
      return $this->output_json (array ('status' => false, 'message' => '更新失敗！', 'content' => LookBanner::$isIsEnabledNames[$banner->is_enabled]));

    return $this->output_json (array ('status' => true, 'message' => '更新成功！', 'content' => LookBanner::$isIsEnabledNames[$banner->is_enabled]));
  }
  ...
  private function _validation_is_enabled_posts (&$posts) {
    if (!(isset ($posts['is_enabled']) && is_numeric ($posts['is_enabled']) && in_array ($posts['is_enabled'], array_keys (LookBanner::$isIsEnabledNames))))
      return '參數錯誤！';
    return '';
  }
}
