...
class Kawashima_stores extends Admin_controller {
  private $uri_1 = null;
  private $store = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'kawashima_stores';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->store = KawashimaStore::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = KawashimaStore::count (array ('conditions' => $conditions));
    ...
    $stores = KawashimaStore::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => 'id DESC',
  ...
  public function create () {
    ...
    $store = null;
    $create = KawashimaStore::transaction (function () use (&$store, $posts) {
      if (!verifyCreateOrm ($store = KawashimaStore::create (array_intersect_key ($posts, KawashimaStore::table ()->columns))))
        return false;
  ...
  public function update () {
    ...
    $store = $this->store;
    $update = KawashimaStore::transaction (function () use ($store, $posts) {
      if (!$store->save ())
        return false;
  ...
  public function destroy () {
    $store = $this->store;
    $delete = KawashimaStore::transaction (function () use ($store) {
  ...
  public function is_enabled ($id = 0) {
    if (!($id && ($store = KawashimaStore::find_by_id ($id, array ('select' => 'id, is_enabled, updated_at')))))
      return $this->output_json (array ('status' => false, 'message' => '當案不存在，或者您的權限不夠喔！'));

    $posts = OAInput::post ();

    if ($msg = $this->_validation_is_enabled_posts ($posts))
      return $this->output_json (array ('status' => false, 'message' => $msg, 'content' => KawashimaStore::$isIsEnabledNames[$store->is_enabled]));

    if ($columns = array_intersect_key ($posts, $store->table ()->columns))
      foreach ($columns as $column => $value)
        $store->$column = $value;

    $update = KawashimaStore::transaction (function () use ($store) { return $store->save (); });

    if (!$update)
      return $this->output_json (array ('status' => false, 'message' => '更新失敗！', 'content' => KawashimaStore::$isIsEnabledNames[$store->is_enabled]));

    return $this->output_json (array ('status' => true, 'message' => '更新成功！', 'content' => KawashimaStore::$isIsEnabledNames[$store->is_enabled]));
  }
  ...
  private function _validation_posts (&$posts) {
    ...
    if (!KawashimaStoreTag::count ())
      return '請先新增分類！';

    if (!(isset ($posts['kawashima_store_tag_id']) && is_numeric ($posts['kawashima_store_tag_id'] = trim ($posts['kawashima_store_tag_id'])) && in_array ($posts['kawashima_store_tag_id'], column_array (KawashimaStoreTag::all (), 'id'))))
      return '請先新增分類！';

    return '';
  }
  private function _validation_is_enabled_posts (&$posts) {
    if (!(isset ($posts['is_enabled']) && is_numeric ($posts['is_enabled']) && in_array ($posts['is_enabled'], array_keys (KawashimaStore::$isIsEnabledNames))))
      return '參數錯誤！';
    return '';
  }
}
