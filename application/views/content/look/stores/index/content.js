...
$(function () {
  ...
  google.maps.event.addDomListener (window, 'load', function () {
    $('.map').map (function () {
      $(this).get (0)._map = new google.maps.Map ($(this).get (0), { draggable: false, zoom: 16, zoomControl: false, scrollwheel: false, scaleControl: false, mapTypeControl: false, navigationControl: false, streetViewControl: false, disableDoubleClickZoom: false, center: new google.maps.LatLng (23.569396231491233, 120.3030703338623)});
      $(this).get (0)._map.mapTypes.set ('map_style', new google.maps.StyledMapType ([{ featureType: 'transit', stylers: [{ visibility: 'simplified' }] }, { featureType: 'poi', stylers: [{ visibility: 'simplified' }] }]));
      $(this).get (0)._map.setMapTypeId ('map_style');
      
      var geocoder = new google.maps.Geocoder ();

      geocoder.geocode ({'address': $(this).data ('address')}, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
          $(this).get (0)._map.setCenter (results[0].geometry.location);
          ...
          new MarkerWithLabel ({
            map: $(this).get (0)._map,
            draggable: false, optimized: false,
            labelContent: $(this).data ('address'),
