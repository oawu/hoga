...
class Look_products extends Admin_controller {
  private $uri_1 = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'look_products';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->product = LookProduct::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = LookProduct::count (array ('conditions' => $conditions));
    ...
    $products = LookProduct::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => 'id DESC',
  ...
  public function create () {
    ...
    $product = null;
    $create = LookProduct::transaction (function () use (&$product, $posts, $big, $small) {
      if (!verifyCreateOrm ($product = LookProduct::create (array_intersect_key ($posts, LookProduct::table ()->columns))))
        return false;
  ...
  public function update () {
    ...
    $product = $this->product;
    $update = LookProduct::transaction (function () use ($product, $posts, $big, $small) {
      if (!$product->save ())
        return false;
  ...
  public function destroy () {
    $product = $this->product;
    $delete = LookProduct::transaction (function () use ($product) {
  ...
  public function is_enabled ($id = 0) {
    if (!($id && ($product = LookProduct::find_by_id ($id, array ('select' => 'id, is_enabled, updated_at')))))
      return $this->output_json (array ('status' => false, 'message' => '當案不存在，或者您的權限不夠喔！'));

    $posts = OAInput::post ();

    if ($msg = $this->_validation_is_enabled_posts ($posts))
      return $this->output_json (array ('status' => false, 'message' => $msg, 'content' => LookProduct::$isIsEnabledNames[$product->is_enabled]));

    if ($columns = array_intersect_key ($posts, $product->table ()->columns))
      foreach ($columns as $column => $value)
        $product->$column = $value;

    $update = LookProduct::transaction (function () use ($product) { return $product->save (); });

    if (!$update)
      return $this->output_json (array ('status' => false, 'message' => '更新失敗！', 'content' => LookProduct::$isIsEnabledNames[$product->is_enabled]));

    return $this->output_json (array ('status' => true, 'message' => '更新成功！', 'content' => LookProduct::$isIsEnabledNames[$product->is_enabled]));
  }
  ...
  private function _validation_is_enabled_posts (&$posts) {
    if (!(isset ($posts['is_enabled']) && is_numeric ($posts['is_enabled']) && in_array ($posts['is_enabled'], array_keys (LookProduct::$isIsEnabledNames))))
      return '參數錯誤！';
    return '';
  }
}
