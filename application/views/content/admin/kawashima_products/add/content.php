  ...
  <table class='table-form'>
    <tbody>
      ...
      <tr>
        <th>敘 述：</th>
        <td>
          <input type='text' name='desc' value='<?php echo isset ($posts['desc']) ? $posts['desc'] : '';?>' placeholder='請輸入敘述..' maxlength='200' pattern='.{1,200}' required title='輸入敘述!'  />
        </td>
      </tr>
      ...
      <?php if ($isIsEnabledNames = KawashimaProduct::$isIsEnabledNames) {
              foreach ($isIsEnabledNames as $key => $name) { ?>
                <option value='<?php echo $key;?>'<?php echo (isset ($posts['is_enabled']) ? $posts['is_enabled'] : KawashimaProduct::NO_ENABLED) == $key ? ' selected': '';?>><?php echo $name;?></option>
        <?php }
            }?>
          </select>
        </td>
