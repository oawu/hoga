...
<?php if ($products) {
        foreach ($products as $product) { ?>
          <tr>
            <td data-title='標題' width='120'><?php echo $product->title;?></td>
            <td data-title='敘述'><?php echo $product->desc;?></td>
            <td data-title='小圖' width='90'><?php echo img($product->small->url ('100x100c'), false, 'class="i_30"');?></td>
            <td data-title='大圖' width='90'><?php echo img($product->big->url ('100x100c'), false, 'class="i_30"');?></td>
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $product->id;?>'<?php echo $product->is_enabled == KawashimaProduct::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo KawashimaProduct::$isIsEnabledNames[$product->is_enabled];?></div>
              </label>
            </td>
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $product->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $product->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
