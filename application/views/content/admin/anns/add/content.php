    ...
    <tbody>
      ...
      <?php if ($brands = Brand::all ()) {
              foreach ($brands as $brand) { ?>
                <option value='<?php echo $brand->id;?>'<?php echo (isset ($posts['brand_id']) ? $posts['brand_id'] : 0) == $brand->id ? ' selected': '';?>><?php echo $brand->name;?></option>
        <?php }
            }?>
          </select>
        </td>
      ...
      <?php if ($isIsEnabledNames = Ann::$isIsEnabledNames) {
              foreach ($isIsEnabledNames as $key => $name) { ?>
                <option value='<?php echo $key;?>'<?php echo (isset ($posts['is_enabled']) ? $posts['is_enabled'] : Ann::NO_ENABLED) == $key ? ' selected': '';?>><?php echo $name;?></option>
        <?php }
            }?>
          </select>
        </td>
